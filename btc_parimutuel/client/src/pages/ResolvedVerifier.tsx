...
import { ExternalLink, CheckCircle, Copy, Link as LinkIcon, FileJson, ArrowRight, X } from "lucide-react";
import { useRoute } from "wouter";
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogTrigger, DialogClose } from "@/components/ui/dialog";
import { MID_DEFAULT } from "@/lib/constants";
import { InvalidMid } from "@/components/InvalidMid";

export default function ResolvedVerifier() {
  const [match, params] = useRoute("/verify/resolved/:mid");
      ...
      <div className="space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 pb-4 border-b border-gray-200">
             <div className="flex items-center gap-4">
                 <Badge className="bg-emerald-600 text-white hover:bg-emerald-700 border-none rounded-sm px-3 py-1 font-mono text-xs uppercase tracking-wider font-bold">
                    RESOLVED
                 </Badge>
        ...
        <div className="border border-slate-200 shadow-sm rounded-sm bg-white overflow-hidden">
            ...
            <div className="p-6 grid grid-cols-2 md:grid-cols-4 gap-y-6 gap-x-8">
                ...
                <div className="md:col-span-2">
                    <div className="text-[10px] uppercase tracking-wider text-slate-400 font-semibold mb-1">Resolution</div>
                    <div className="font-mono text-sm text-slate-900">2026-04-01T09:15:22Z</div>
                </div>
            </div>
        </div>
        ...
        <div className="space-y-3">
             ...
             <div className="border border-slate-200 rounded-sm bg-white divide-y divide-slate-100">
                  ...
                  <a href="#" className="flex items-center justify-between p-4 hover:bg-slate-50 transition-colors group">
                      <div className="flex items-center gap-3">
                          <span className="w-2 h-2 rounded-full bg-slate-300 group-hover:bg-blue-500 transition-colors"></span>
                          <span className="text-sm font-medium text-slate-700 group-hover:text-blue-700">Settle / Close tx</span>
                      </div>
                      <div className="flex items-center gap-3">
                  ...
                  <a href="#" className="flex items-center justify-between p-4 hover:bg-slate-50 transition-colors group">
                      <div className="flex items-center gap-3">
                          ...
                          <span className="text-sm font-medium text-slate-900 group-hover:text-blue-700">Resolve tx</span>
                      </div>
                      <div className="flex items-center gap-3">
                          <span className="font-mono text-xs text-slate-400">4zR3...YSHS</span>
                          <Copy className="w-3.5 h-3.5 text-slate-300 hover:text-slate-500" />
                          <ExternalLink className="w-3.5 h-3.5 text-slate-300 group-hover:text-blue-400" />
                      </div>
                  </a>
                  <div className="p-4 bg-slate-50/50 space-y-3">
                       <div className="flex items-center justify-between text-xs font-mono text-slate-500">
                           <span>Receipt init txs</span>
                           
                           <Dialog>
                                <DialogTrigger asChild>
                                    <button className="text-blue-600 hover:underline cursor-pointer">View list (12)</button>
                                </DialogTrigger>
                                <DialogContent className="max-w-xl">
                                    <DialogHeader>
                                        <DialogTitle>Receipt init txs (w02–w11)</DialogTitle>
                                    </DialogHeader>
                                    <div className="mt-4">
                                        ...
                                        <div className="space-y-2 max-h-[300px] overflow-y-auto">
                                            {Array.from({length: 12}).map((_, i) => (
                                                <div key={i} className="flex items-center justify-between p-2 border border-slate-100 rounded bg-white text-xs font-mono">
                                                    <span className="text-slate-600">w{String(i + 2).padStart(2, '0')}</span>
                                                    <span className="text-slate-400">4zR3...YSHS</span>
                       ...
                       <div className="flex items-center justify-between text-xs font-mono text-slate-500">
                           ...
                           <Dialog>
                                <DialogTrigger asChild>
                                    <button className="text-blue-600 hover:underline cursor-pointer">View list (6)</button>
                                </DialogTrigger>
                                <DialogContent className="max-w-xl">
                                    <DialogHeader>
                                        <DialogTitle>Claim txs (w02–w06)</DialogTitle>
                                    </DialogHeader>
                                    <div className="mt-4">
                                        <div className="bg-slate-50 p-2 text-[10px] text-slate-500 font-mono mb-2 border border-slate-100">
                                            Ordered by slot asc.
