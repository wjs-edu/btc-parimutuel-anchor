...
const proofFactsHtml=(st)=>{const o=(st&&st.obj)||{};
  const schema=o.schema_version||"vfinal-p0";
  const rule=o.rule_version||"vFinal";
  const params=o.params_hash||"not_captured_for_legacy_demo";
  const oracle=o.oracle_reference||"not_captured_for_legacy_demo";
  const commitClose=o.commit_close_ts||"not_captured_for_legacy_demo";
  const resolution=o.resolution_ts||"not_captured_for_legacy_demo";
  return `<div class="card"><div class="h">Proof facts (canonical)</div>`+
    `<div class="row"><div class="k">schema_version</div><div style="font-weight:800">${esc(schema)}</div></div>`+
    `<div class="row"><div class="k">rule_version</div><div style="font-weight:800">${esc(rule)}</div></div>`+
    `<div class="row"><div class="k">params_hash</div><div style="font-weight:800">${esc(params)}</div></div>`+
    `<div class="row"><div class="k">oracle_reference</div><div style="font-weight:800">${esc(oracle)}</div></div>`+
    `<div class="row"><div class="k">commit_close_ts</div><div style="font-weight:800">${esc(commitClose)}</div></div>`+
    `<div class="row"><div class="k">resolution_ts</div><div style="font-weight:800">${esc(resolution)}</div></div>`+
    `<div class="muted" style="margin-top:8px">Legacy demo markets may show not_captured_for_legacy_demo for fields not recorded historically.</div>`+
  `</div>`;};
const verifyResolvedHtml=(id,st)=>{const dir=path.join(root,`artifacts/resolved/${id}`);const need=["resolve.sig.txt","claim.sig.txt"];if(!need.every(f=>exists(path.join(dir,f))))return null;const r=read(path.join(dir,"resolve.sig.txt")).trim();const c=read(path.join(dir,"claim.sig.txt")).trim();return `<h1>Verifier: RESOLVED Market ${esc(id)}</h1><div class="card">${summaryRows(st)}${proofFactsHtml(st)}<div style="margin-top:10px"><div class="h">Labeled transaction links</div><ul><li>Publish: <span class="muted">Not captured for legacy demo market</span></li><li>Commit: <span class="muted">Not captured for legacy demo market</span></li><li>Close: <span class="muted">Not captured for legacy demo market</span></li><li>Resolve: <a href="${tx(r)}">${esc(r)}</a></li><li>Claim: <a href="${tx(c)}">${esc(c)}</a></li><li>status (canonical): <a href="/status/${esc(id)}.json">/status/${esc(id)}.json</a></li></ul></div></div>${provesHtml()}${copyHtml()}<details><summary>Raw status JSON</summary><pre>${esc(JSON.stringify(st.obj,null,2))}</pre></details><div class="card"><div class="h">Market snapshot (raw)</div><div class="muted">The verifier does not embed account data blobs in HTML evidence.</div><ul><li><a href="/raw/market/resolved/${esc(id)}.json">/raw/market/resolved/${esc(id)}.json</a></li></ul></div>`;};
const verifyCanceledHtml=(id,st)=>{const dir=path.join(root,`artifacts/canceled/${id}`);const need=["close.sig.txt"];if(!need.every(f=>exists(path.join(dir,f))))return null;const close=read(path.join(dir,"close.sig.txt")).trim();return `<h1>Verifier: CANCELED Market ${esc(id)}</h1><div class="card">${summaryRows(st)}${proofFactsHtml(st)}<div style="margin-top:10px"><div class="h">Labeled transaction links</div><ul><li>close (canceled): <a href="${tx(close)}">${esc(close)}</a></li><li>status (canonical): <a href="/status/${esc(id)}.json">/status/${esc(id)}.json</a></li></ul></div></div>${provesHtml()}${copyHtml()}<details><summary>Raw status JSON</summary><pre>${esc(JSON.stringify(st.obj,null,2))}</pre></details><div class="card"><div class="h">Market snapshot (raw)</div><div class="muted">The verifier does not embed account data blobs in HTML evidence.</div><ul><li><a href="/raw/market/canceled/${esc(id)}.json">/raw/market/canceled/${esc(id)}.json</a></li></ul></div>`;};
